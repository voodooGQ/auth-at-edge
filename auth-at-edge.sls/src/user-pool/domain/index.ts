...
async function ensureCognitoUserPoolDomain(
  ...
  const decodedPhysicalResourceId = decodePhysicalResourceId(
    physicalResourceId!,
  );
  ...
  if (action === "Delete") {
    if (decodedPhysicalResourceId) {
      const {
        ...
        domainPrefix: oldDomainPrefix,
      } = decodedPhysicalResourceId;
    ...
    const randomValue =
      (decodedPhysicalResourceId && decodedPhysicalResourceId.randomValue) ||
      randomBytes(4).toString("hex");
    const domainPrefix = `auth-${randomValue}`;
    const existingDomain = await COGNITO_CLIENT.describeUserPoolDomain({
      Domain: domainPrefix,
    }).promise();
    if (
      action === "Create" ||
...
export const handler: CloudFormationCustomResourceHandler = async event => {
  ...
  try {
    const {
      domainName,
      physicalResourceId,
    } = await ensureCognitoUserPoolDomain(
      RequestType,
...
function encodePhysicalResourceId(
  userPoolId: string,
  domainPrefix: string,
  randomValue: string,
) {
...
function decodePhysicalResourceId(physicalResourceId: string) {
  console.log("IN DECODE_PHYSICAL_RESOURCE_ID");
  console.log("physicalResourceId");
  console.log(physicalResourceId);
  try {
    return JSON.parse(physicalResourceId) as {
      userPoolId: string;
      domainPrefix: string;
      randomValue: string;
    };
